...
import { getDefaultExpirationContext, getExpirationContextOptions, makeSingleExpirationSurface } from "../../../lib/trader-edge-context";
...
function firstExpirationFromSurface(surface: OptionSurfaceSnapshot | null): string {
  return getDefaultExpirationContext(surface);
}
...
async function loadSupabaseSurface(nextTicker = ticker) {
  ...
  const expirationOptions = useMemo(() => getExpirationContextOptions(selectedFullSurface), [selectedFullSurface]);
...
: "No OI surface snapshot saved"}
        ...
        <p style={{ color: wheelColors.teal, margin: "0.5rem 0 0", fontSize: 12, fontWeight: 800 }}>
          Trader Edge is canonical for the selected Supabase surface date + expiration chain. Control Center, Watchlist, and Wheel should match when the same chain context is selected.
        </p>
      ...
      <section style={{ border: `1px solid ${wheelColors.border}`, borderRadius: 8, padding: "1rem", background: wheelColors.panel }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "0.75rem", fontSize: 14 }}>
  ...
  {decision.positionContext.managedLegs.length === 0 ? (
    ...
    <div style={{ display: "grid", gap: "0.75rem" }}>
      {decision.positionContext.managedLegs.map((leg) => (
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(110px, 1fr))", gap: "0.5rem", fontSize: 13 }}>
          ...
          <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.5rem", fontSize: 12 }}>
