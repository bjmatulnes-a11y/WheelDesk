...
function normalizeCandleTime(candle: any, timeframe: string): string {
  const raw = candle?.datetime ?? candle?.timestamp ?? candle?.time ?? candle?.date ?? "";
  if (!raw) return "";
...
function rowOpenInterest(row: any): number {
  const candidates = [row?.openInterest, row?.open_interest, row?.oi, row?.raw?.openInterest, row?.raw?.open_interest, row?.raw?.oi];
...
function chainScore(chain: any): number | null {
  const candidates = [chain?.summary?.prevailingScore, chain?.summary?.score, chain?.summary?.edgeScore, chain?.prevailingScore, chain?.score];
  for (const candidate of candidates) {
    const n = Number(candidate);
    if (Number.isFinite(n)) return n;
  }
  ...
  const chains = rawChains
    .map((chain: any) => {
      ...
      return {
        ...
        dteAtCapture: chain?.dteAtCapture ?? chain?.dte ?? dteFromExpiration(expiration, snapshotDate) ?? null,
      };
    })
    .filter(Boolean);
...
function extractSnapshots(payload: any): OptionSurfaceSnapshot[] {
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) {
      return candidate.map(normalizeSurfaceSnapshot).filter((snapshot): snapshot is OptionSurfaceSnapshot => Boolean(snapshot));
    }
  }

  const normalized = normalizeSurfaceSnapshot(payload?.snapshot ?? payload?.surface ?? payload);
  return normalized ? [normalized] : [];
}

async function fetchSupabaseSurfaces(ticker: string): Promise<OptionSurfaceSnapshot[]> {
  const response = await fetch(`/api/supabase/surface-snapshot?ticker=${encodeURIComponent(ticker)}&limit=120`, {
    cache: "no-store",
  });
...
function makeSingleChainSurface(surface: OptionSurfaceSnapshot | null, chain: any | null): OptionSurfaceSnapshot | null {
  if (!surface || !chain) return null;
  return { ...surface, chains: [chain] } as OptionSurfaceSnapshot;
}

function findMatchingExpirationSurface(surface: OptionSurfaceSnapshot | null, expiration: string): OptionSurfaceSnapshot | null {
...
function ChartOverlayControls({
  ...
  return (
    <section style={{ ...cardStyle, padding: "0.85rem 1rem", display: "grid", gap: "0.75rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.75rem", flexWrap: "wrap" }}>
        <div>
          <div style={{ color: colors.text, fontSize: 13, fontWeight: 950 }}>Chart Room overlays</div>
          <div style={{ color: colors.muted, fontSize: 12 }}>Use this tab as the dedicated visual workspace while Control Center stays open.</div>
        </div>
        <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
          <button type="button" onClick={() => setOverlays(defaultOverlayFlags)} style={buttonStyle}>Default</button>
          <button type="button" onClick={() => setOverlays(allOverlayFlags)} style={buttonStyle}>All overlays</button>
          <button type="button" onClick={() => setOverlays(candleOnlyOverlayFlags)} style={buttonStyle}>Candles only</button>
        </div>
      </div>

      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", alignItems: "center" }}>
        <Toggle checked={overlays.prevailingSurfaceLevels} label="Prevailing levels" onChange={(checked) => setOverlays((current) => ({ ...current, prevailingSurfaceLevels: checked }))} />
        <Toggle checked={overlays.selectedChainLevels} label="Chain OI levels" onChange={(checked) => setOverlays((current) => ({ ...current, selectedChainLevels: checked }))} />
        <Toggle checked={overlays.selectedChainPath} label="OI path" onChange={(checked) => setOverlays((current) => ({ ...current, selectedChainPath: checked }))} />
        <Toggle checked={overlays.selectedChainIvSurface} label="IV band" onChange={(checked) => setOverlays((current) => ({ ...current, selectedChainIvSurface: checked }))} />
        <Toggle checked={overlays.dealerPressure} label="Dealer pressure" onChange={(checked) => setOverlays((current) => ({ ...current, dealerPressure: checked }))} />
        <Toggle checked={overlays.wallMigration} label="Wall migration" onChange={(checked) => setOverlays((current) => ({ ...current, wallMigration: checked }))} />
        <Toggle checked={overlays.flowIntelligence} label="Flow intelligence" onChange={(checked) => setOverlays((current) => ({ ...current, flowIntelligence: checked }))} />
      </div>
    </section>
  );
}
...
function MetricPill({ label, value, tone = colors.teal }: { label: string; value: string; tone?: string }) {
  return (
    <div style={{ ...cardStyle, padding: "0.85rem" }}>
      ...
      <div style={{ color: tone, fontSize: 22, fontWeight: 950, marginTop: 4 }}>{value}</div>
    </div>
  );
}
...
function ChartRoomContent() {
  ...
  const initialTicker = normalizeTicker(searchParams.get("ticker") ?? searchParams.get("symbol")) || "SOFI";
  const initialSurfaceDate = String(searchParams.get("surfaceDate") ?? searchParams.get("surface") ?? "").slice(0, 10);
  const initialExpiration = String(searchParams.get("expiration") ?? "");
  const initialTimeframe = String(searchParams.get("tf") ?? searchParams.get("timeframe") ?? "daily");

  const [mounted, setMounted] = useState(false);
  const [ticker, setTicker] = useState(initialTicker);
  const [tickerInput, setTickerInput] = useState(initialTicker);
  const [surfaceSnapshots, setSurfaceSnapshots] = useState<OptionSurfaceSnapshot[]>([]);
  const [selectedSurfaceDate, setSelectedSurfaceDate] = useState(initialSurfaceDate);
  const [selectedExpiration, setSelectedExpiration] = useState(initialExpiration);
  const [candleTimeframe, setCandleTimeframe] = useState<string>(TIMEFRAMES.includes(initialTimeframe as any) ? initialTimeframe : "daily");
    ...
    async function loadSurfaces() {
      ...
      try {
        ...
        setSelectedSurfaceDate((current) => {
          ...
          if (initialSurfaceDate && loaded.some((surface) => surface.snapshotDate === initialSurfaceDate)) return initialSurfaceDate;
          return loaded[0]?.snapshotDate ?? "";
        });

        setStatus(loaded.length ? `Loaded ${loaded.length} Supabase surface(s) for ${ticker}.` : `No Supabase surface found for ${ticker}.`);
      } catch (error) {
        if (cancelled) return;
        setSurfaceSnapshots([]);
  ...
  useEffect(() => {
    ...
    async function loadCandles() {
      ...
      try {
        const series = await getPriceSeries(requestedTicker, requestedTimeframe as Timeframe);
  ...
  const selectedSurface = useMemo(() => {
    ...
    return surfaceSnapshots.find((surface) => surface.snapshotDate === selectedSurfaceDate) ?? surfaceSnapshots[0] ?? null;
  ...
  useEffect(() => {
    ...
    setSelectedExpiration((current) => {
      ...
      if (initialExpiration && expirationOptions.some((item) => item.expiration === initialExpiration)) return initialExpiration;
  ...
  const selectedChain = useMemo(() => {
    ...
    return (selectedSurface.chains as any[]).find((chain) => expirationOf(chain) === selectedExpiration) ?? selectedSurface.chains[0] ?? null;
  }, [selectedSurface, selectedExpiration]);

  const selectedChainSurface = useMemo(() => makeSingleChainSurface(selectedSurface, selectedChain), [selectedSurface, selectedChain]);

  const priorFullSurface = useMemo(() => {
    if (!selectedSurface) return null;
    return findPriorSurfaceForTicker(surfaceSnapshots, selectedSurface.ticker, selectedSurface.snapshotDate);
  }, [selectedSurface, surfaceSnapshots]);

  const priorSelectedChainSurface = useMemo(() => findMatchingExpirationSurface(priorFullSurface, selectedExpiration), [priorFullSurface, selectedExpiration]);

  const analysisPrice = useMemo(() => {
    const lastClose = candles.length ? candles[candles.length - 1]?.close : null;
    const surfaceClose = Number(selectedSurface?.price?.close ?? selectedSurface?.dailyStructure?.spot ?? 0);
    return Number.isFinite(surfaceClose) && surfaceClose > 0 ? surfaceClose : Number(lastClose ?? 0);
  }, [candles, selectedSurface]);

  const surfaceTraderEdge = useMemo(() => {
    if (!selectedSurface) return null;
    return buildTraderEdgeSummary({ ticker, surface: selectedSurface, candles, livePrice: analysisPrice });
  }, [ticker, selectedSurface, candles, analysisPrice]);

  const chainTraderEdge = useMemo(() => {
    if (!selectedChainSurface) return null;
    return buildTraderEdgeSummary({ ticker, surface: selectedChainSurface, candles, livePrice: analysisPrice });
  }, [ticker, selectedChainSurface, candles, analysisPrice]);

  const chainWallMigration = useMemo(
    () => buildWallMigrationSummary({ currentSurface: selectedChainSurface, priorSurface: priorSelectedChainSurface }),
    [selectedChainSurface, priorSelectedChainSurface]
  );

  const chainDealerPressure = useMemo(
    () => buildDealerPressureSummary({ surface: selectedChainSurface, edge: chainTraderEdge, wallMigration: chainWallMigration, candles, livePrice: analysisPrice }),
    [selectedChainSurface, chainTraderEdge, chainWallMigration, candles, analysisPrice]
  );

  const chainProjectionReport = useMemo(
    () => buildOIProjectionReport({ snapshot: toChainSnapshot(selectedChainSurface), currentPrice: analysisPrice }),
    [selectedChainSurface, analysisPrice]
  );

  const chainOIPath = useMemo(
    () => buildOIImpliedPath({ projectionReport: chainProjectionReport, edgeSummary: chainTraderEdge, wallMigration: chainWallMigration, currentPrice: analysisPrice }),
    [chainProjectionReport, chainTraderEdge, chainWallMigration, analysisPrice]
  );

  const predictiveMatrix = useMemo(
    () => buildPredictiveMatrix({ path: chainOIPath, dealerPressure: chainDealerPressure, edgeSummary: chainTraderEdge, wallMigration: chainWallMigration }),
  ...
  }, [selectedChainSurface, analysisPrice, expirationOptions, selectedExpiration, candles]);

  const flowIntelligence = useMemo(
    () => buildFlowIntelligenceView({ surface: selectedChainSurface, currentPrice: analysisPrice }),
    [selectedChainSurface, analysisPrice]
  );

  const chainOIIntelligence = useMemo(
    () => buildOIIntelligenceView({ surface: selectedChainSurface, currentPrice: analysisPrice }),
    [selectedChainSurface, analysisPrice]
  );
  ...
  const chartMatrix = overlays.dealerPressure || overlays.wallMigration ? predictiveMatrix : null;
  ...
  return (
    <AuthGate>
      ...
      >
        <div style={{ maxWidth: 1840, margin: "0 auto", display: "grid", gap: "1rem" }}>
          <header style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "flex-start", flexWrap: "wrap" }}>
            <div>
              <div style={{ color: colors.teal, fontSize: 12, fontWeight: 950, letterSpacing: "0.12em", textTransform: "uppercase" }}>WheelDesk Chart Room</div>
              <h1 style={{ color: colors.text, margin: "0.3rem 0 0", fontSize: "clamp(2rem, 4vw, 4.2rem)", letterSpacing: "-0.06em" }}>
                {ticker} structure chart
              </h1>
              <p style={{ color: colors.muted, margin: "0.35rem 0 0", maxWidth: 850 }}>
                Dedicated large-screen chart workspace. Keep Control Center open in the original tab while this view runs the chart, OI path, IV band, dealer pressure, wall migration, and flow overlays.
              </p>
            </div>
          ...
          <section style={{ ...cardStyle, padding: "0.9rem 1rem", display: "grid", gap: "0.85rem" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 190px), 1fr))", gap: "0.75rem", alignItems: "end" }}>
              <label style={{ display: "grid", gap: 6, color: colors.muted, fontSize: 12, fontWeight: 900 }}>
                Ticker
                <div style={{ display: "flex", gap: "0.45rem" }}>
                  ...
                  <button
                    ...
                    style={{ border: "1px solid rgba(34, 211, 238, 0.35)", background: "rgba(34, 211, 238, 0.12)", color: colors.teal, borderRadius: 12, padding: "0 0.75rem", fontWeight: 950, cursor: "pointer" }}
                  >
                    Load
                  </button>
                </div>
              ...
              <label style={{ display: "grid", gap: 6, color: colors.muted, fontSize: 12, fontWeight: 900 }}>
                Surface Date
                <select value={selectedSurfaceDate} onChange={(event) => setSelectedSurfaceDate(event.target.value)} style={{ border: "1px solid rgba(148, 163, 184, 0.24)", background: "rgba(15, 23, 42, 0.88)", color: colors.text, borderRadius: 12, padding: "0.68rem 0.75rem", fontWeight: 850 }}>
                  {!surfaceSnapshots.length ? <option value="">No surfaces</option> : null}
                  {surfaceSnapshots.map((surface) => <option key={surface.snapshotDate} value={surface.snapshotDate}>{surface.snapshotDate}</option>)}
                </select>
              </label>

              <label style={{ display: "grid", gap: 6, color: colors.muted, fontSize: 12, fontWeight: 900 }}>
                Expiration Chain
                <select value={selectedExpiration} onChange={(event) => setSelectedExpiration(event.target.value)} style={{ border: "1px solid rgba(148, 163, 184, 0.24)", background: "rgba(15, 23, 42, 0.88)", color: colors.text, borderRadius: 12, padding: "0.68rem 0.75rem", fontWeight: 850 }}>
                  ...
                  {expirationOptions.map((item) => (
                    <option key={item.expiration} value={item.expiration}>
                      {item.expiration}{item.dte != null ? ` | ${item.dte}D` : ""}{item.dominanceScore != null ? ` | dom ${safeFixed(item.dominanceScore, 1)}` : ""}
                    </option>
                  ))}
                </select>
              </label>
              ...
              <label style={{ display: "grid", gap: 6, color: colors.muted, fontSize: 12, fontWeight: 900 }}>
                Candles
                <select value={candleTimeframe} onChange={(event) => setCandleTimeframe(event.target.value)} style={{ border: "1px solid rgba(148, 163, 184, 0.24)", background: "rgba(15, 23, 42, 0.88)", color: colors.text, borderRadius: 12, padding: "0.68rem 0.75rem", fontWeight: 850 }}>
            ...
            <div style={{ color: colors.muted, fontSize: 12 }}>
              {surfaceLoading || candleLoading ? "Loading chart room data..." : status}
            </div>
          </section>

          <ChartOverlayControls overlays={overlays} setOverlays={setOverlays} />

          <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 180px), 1fr))", gap: "0.75rem" }}>
            <MetricPill label="Current / Analysis" value={fmt(analysisPrice)} tone={colors.green} />
            <MetricPill label="Surface" value={selectedSurface?.snapshotDate ?? "N/A"} tone={colors.text} />
            <MetricPill label="Expiration" value={selectedExpiration || "N/A"} tone={colors.teal} />
            <MetricPill label="Rows" value={String(selectedChainSurface?.chains?.[0]?.rows?.length ?? 0)} tone={colors.teal} />
          ...
          {selectedSurface ? (
            ...
            <section style={{ ...cardStyle, padding: "1.25rem" }}>
              ...
              <p style={{ color: colors.muted }}>
                Chart Room needs a saved option surface from Supabase. Load/harvest a ticker first, then open Chart Room from Control Center.
              </p>
              <Link href="/dashboard" style={{ color: colors.teal, fontWeight: 950 }}>Open Dashboard Harvest</Link>
            </section>
          )}
        </div>
      </main>
...
export default function ControlCenterChartRoomPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: colors.bg, color: colors.text, padding: "2rem" }}>Loading Chart Room...</div>}>
      <ChartRoomContent />
    </Suspense>
  );
}
