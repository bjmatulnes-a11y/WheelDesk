...
export type OIImpliedPathResult = {
  ...
  forecastMode?: "classic_oi_path" | "smooth_pressure_path";
  maxDte?: number | null;
...
function uniquePath(points: OIPathPoint[]): OIPathPoint[] {
  ...
  for (const point of points) {
    ...
    byDate.set(point.date.slice(0, 10), {
      date: point.date.slice(0, 10),
      value: roundToCents(point.value),
    });
  }
...
function projectionPathPoints(
  projection: OIProjectionReport | null | undefined,
  field: "adjustedCenter" | "upperRange" | "lowerRange",
): OIPathPoint[] {
  return uniquePath(
    (projection?.points ?? [])
      .filter(
        (point) => point.expiration && Number.isFinite(Number(point[field])),
      )
      .map((point) => ({
...
function projectionHorizonSessions(
  projection: OIProjectionReport | null | undefined,
  fallback: number,
): number {
  const maxPointDte = Math.max(
    0,
    ...(projection?.points ?? [])
  ...
  if (maxPointDte > 0)
    return Math.max(5, Math.min(60, Math.round(maxPointDte)));
  const maxDte = Number((projection as any)?.maxDte);
  if (Number.isFinite(maxDte) && maxDte > 0)
    return Math.max(5, Math.min(60, Math.round(maxDte)));
  return fallback;
}
...
function weightedAverage(
  ...
  return (
    clean.reduce((sum, item) => sum + item.value * item.weight, 0) / weight
  );
}
...
function confidenceFrom(args: {
  ...
  let score =
    args.projectionConfidence === "high"
      ? 68
      : args.projectionConfidence === "medium"
        ? 55
        : 42;
  ...
  if ((args.edge?.staleDays ?? 0) >= 2)
    args.confidenceDegraders.push(
      "Surface is stale; OI path confidence is reduced.",
    );
  if (args.edge?.volumeThrust == null)
    args.confidenceDegraders.push(
      "Current volume confirmation is unavailable; treat path as structure-only.",
    );
  if ((args.edge?.trapRisk ?? 0) >= 75)
    args.confidenceDegraders.push(
      "Trap risk is high; avoid reading the path as a clean directional forecast.",
    );
  if (!args.wallMigration?.hasPrior)
    args.confidenceDegraders.push(
      "No prior surface comparison; wall migration cannot confirm direction.",
    );
...
function regimeFrom(args: {
  ...
  if (edge?.magnet != null && edge.analysisPrice) {
    const magnetDistancePct =
      Math.abs((edge.magnet - edge.analysisPrice) / edge.analysisPrice) * 100;
    if (magnetDistancePct <= 8) return "magnet_pull";
  }

  if (args.projection?.projectedBias === "bullish")
    return "bullish_unlock_watch";
  if (args.projection?.projectedBias === "bearish")
    return "bearish_failure_watch";
  return "mixed";
}
...
function activeScenarioFrom(args: {
  ...
  if (args.invalidAbove != null && args.currentPrice >= args.invalidAbove)
    return "bullish_unlock";
  if (args.invalidBelow != null && args.currentPrice <= args.invalidBelow)
...
function buildTradePermissions(
  edge?: TraderEdgeSummary | null,
): OITradePermissions {
  const ccFloor =
    edge?.executableCoveredCallFloor != null
      ? edge.executableCoveredCallFloor.toFixed(2)
      : "outside resistance";
  const cspCeiling =
    edge?.executableCspCeiling != null
      ? edge.executableCspCeiling.toFixed(2)
      : "below support";
  const compression = edge?.compressionState ?? "Open / not compressed";
  const trapRisk = edge?.trapRisk ?? 0;

  const newPremium =
    trapRisk >= 65 || compression !== "Open / not compressed"
...
function baseTargetFrom(args: {
  ...
  const weightedProjection = weightedAverage(
    (projection?.points ?? [])
      .slice(0, 5)
      .map((point) => ({ value: point.adjustedCenter, weight: point.weight })),
  );
  ...
  if (args.regime === "pin_chop" || args.regime === "magnet_pull") {
    ...
    const rangeMid =
      edge?.support != null && edge.resistance != null
        ? (edge.support + edge.resistance) / 2
        : magnet;
    target = magnet * 0.65 + rangeMid * 0.25 + target * 0.1;
  }
...
function getEnvelopeWidth(args: {
  ...
  const atrWidth =
    args.edge?.atrPct != null
      ? currentPrice * clamp((args.edge.atrPct / 100) * 0.95, 0.012, 0.065)
      : null;
  const rangeWidth =
    args.edge?.support != null && args.edge.resistance != null
      ? Math.abs(args.edge.resistance - args.edge.support)
      : null;
...
function buildSmoothPath(args: {
  ...
  for (let i = 1; i <= args.horizonSessions; i += 1) {
    ...
    const shapedT =
      args.curvature === "late"
        ? smoothStep(t)
        : args.curvature === "linear"
          ? t
          : easeOutCubic(t);
    const raw =
      args.startValue + (args.targetValue - args.startValue) * shapedT;
    const value = clampStep(raw, previous, args.maxStep);
    previous = value;
    points.push({
      date: addBusinessDays(args.startDate, i),
...
function buildActivatedScenarioPath(args: {
  ...
  points.push({
    date: addBusinessDays(args.startDate, 1),
    value: roundToCents(args.activationValue),
  });

  for (let i = 2; i <= sessions; i += 1) {
    const t = (i - 1) / Math.max(1, sessions - 1);
    const shapedT =
      args.curvature === "early"
        ? easeOutCubic(t)
        : args.curvature === "linear"
          ? t
          : smoothStep(t);
    const raw =
      args.activationValue +
      (args.targetValue - args.activationValue) * shapedT;
    const value = clampStep(raw, previous, args.maxStep);
    previous = value;
    points.push({
      date: addBusinessDays(args.startDate, i),
...
function buildActionMatrix(args: {
  ...
  const rangeText =
    edge?.support != null && edge.resistance != null
      ? `${edge.support.toFixed(2)} – ${edge.resistance.toFixed(2)}`
      : "active OI range";
  const ccFloor =
    edge?.executableCoveredCallFloor != null
      ? edge.executableCoveredCallFloor.toFixed(2)
  ...
  const cspCeiling =
    edge?.executableCspCeiling != null
      ...
      : "below support";
  const above =
    args.invalidAbove != null ? args.invalidAbove.toFixed(2) : "resistance";
  const below =
    args.invalidBelow != null ? args.invalidBelow.toFixed(2) : "support";
  ...
  return [
    {
      ...
      existingPositionAction:
        "Manage existing exposure; avoid stacking new premium inside the active OI range.",
    },
    {
      scenario: "Bullish unlock",
      condition: `Price accepts above ${above}.`,
      coveredCallAction:
        "Treat the old call wall as possible fuel; avoid tight covered calls or roll higher.",
      cspAction:
        "CSPs can improve only if support/magnet migrate higher and premium remains worth the risk.",
      existingPositionAction:
        "Let long calls/upside work; defend short calls if price holds above the unlock rail.",
    },
    {
      scenario: "Bearish failure",
      condition: `Price loses ${below}.`,
      coveredCallAction:
        "Covered calls become cleaner only after failed support confirms and bounce risk fades.",
      cspAction:
        "Pause or move CSPs lower; do not treat the broken put wall as safe support.",
      existingPositionAction:
        "Defend short puts and reduce bullish exposure until a new support wall forms.",
    },
  ];
}
...
export function buildOIImpliedPath(args: {
  ...
  const bufferPct =
    edge?.compressionState === "High compression" ? 0.004 : 0.006;
  const buffer = Math.max(0.05, currentPrice * bufferPct);
  const invalidAbove =
    edge?.resistance != null ? roundToCents(edge.resistance + buffer) : null;
  const invalidBelow =
    edge?.support != null ? roundToCents(edge.support - buffer) : null;
  const magnet = edge?.magnet ?? null;

  const projectionPoints = projection?.points ?? [];
  const maxWeightPoint = projectionPoints.length
    ? projectionPoints.reduce(
        (best, point) => (point.weight > best.weight ? point : best),
        projectionPoints[0],
      )
    : null;
  ...
  const smoothHorizonSessions = getHorizonSessions({
    edge,
    wallMigration: wall,
  });
  const startDate =
  ...
  const envelopeWidth = getEnvelopeWidth({
    currentPrice,
    edge,
    wallMigration: wall,
  });
  ...
  const chainMagnetBasePath = projectionPathPoints(
    projection,
    "adjustedCenter",
  );
  const chainMagnetUpperBand = projectionPathPoints(projection, "upperRange");
  const chainMagnetLowerBand = projectionPathPoints(projection, "lowerRange");
  const useChainMagnetPath = chainMagnetBasePath.length >= 2;
  const horizonSessions = useChainMagnetPath
    ? projectionHorizonSessions(projection, smoothHorizonSessions)
    : smoothHorizonSessions;

  const basePath = useChainMagnetPath
    ? chainMagnetBasePath
    : buildSmoothPath({
        startDate,
  ...
  const upperBand =
    useChainMagnetPath && chainMagnetUpperBand.length
      ? chainMagnetUpperBand
  ...
  const lowerBand =
    useChainMagnetPath && chainMagnetLowerBand.length
      ? chainMagnetLowerBand
      : basePath.map((point, index) => {
          const t = (index + 1) / horizonSessions;
          const width = envelopeWidth * (0.65 + 0.35 * t);
          return { date: point.date, value: roundToCents(point.value - width) };
  ...
  const scenarioAmp =
    confidence === "high" ? 1 : confidence === "medium" ? 0.72 : 0.52;
  ...
  const bullishExtension =
    Math.max(envelopeWidth * 1.5, activeRange * 0.16, currentPrice * 0.018) *
    scenarioAmp;
  ...
  if (useChainMagnetPath) {
    notes.push(
      `Classic OI path is built from ${chainMagnetBasePath.length} expiration-chain OI centers${projection?.maxDte != null ? ` inside ${Math.round(projection.maxDte)}D` : ""}.`,
    );
  }
  const activeScenario = activeScenarioFrom({
    currentPrice,
    invalidAbove,
    invalidBelow,
  });
  ...
  if (regime === "pin_chop") {
    notes.push(
      "Base case treats the active OI range as a pin/chop pressure zone until support or resistance breaks.",
    );
  } else if (regime === "magnet_pull") {
    notes.push(
      "Base case favors mean reversion toward the OI magnet while support/resistance remain intact.",
    );
  } else if (regime === "bullish_unlock_watch") {
    notes.push(
      "Upside scenario has higher weight because options positioning or wall migration leans higher.",
    );
  } else if (regime === "bearish_failure_watch") {
    notes.push(
      "Downside scenario has higher weight because support or wall migration is weakening.",
    );
  } else if (regime === "expansion") {
    notes.push(
      "OI range is widening; treat the path as a scenario envelope, not a tight forecast.",
    );
  } else {
    notes.push(
      "Current OI structure is mixed; use rails and strike zones more than the base line.",
    );
  }

  notes.push(
    `Path horizon is intentionally limited to ${horizonSessions} trading sessions to avoid fake long-range precision.`,
  );

  triggerNotes.push(
    `${activeScenarioLabel(activeScenario)}. Green/red scenarios remain inactive unless their rails are triggered.`,
  );
  if (invalidAbove != null)
    triggerNotes.push(
      `Bullish unlock above ${invalidAbove.toFixed(2)}; the green scenario path starts at this rail and is inactive until price accepts above it.`,
    );
  if (invalidBelow != null)
    triggerNotes.push(
      `Bearish failure below ${invalidBelow.toFixed(2)}; the red scenario path starts at this rail and is inactive until price loses it.`,
    );
  if (magnet != null)
    triggerNotes.push(
      `OI magnet reference sits at ${magnet.toFixed(2)}; base case can mean-revert toward this level while structure holds.`,
    );

  if (wall?.hasPrior) {
    migrationNotes.push(`${wall.label}: ${wall.interpretation}`);
    if (wall.resistanceChange != null)
      migrationNotes.push(
        `Resistance change: ${wall.resistanceChange >= 0 ? "+" : ""}${wall.resistanceChange.toFixed(2)}.`,
      );
    if (wall.supportChange != null)
      migrationNotes.push(
        `Support change: ${wall.supportChange >= 0 ? "+" : ""}${wall.supportChange.toFixed(2)}.`,
      );
  } else {
    migrationNotes.push(
      "No prior saved surface was available, so path is based on current OI structure only.",
    );
  }

  if (edge?.executableCoveredCallFloor != null)
    tradePlanNotes.push(
      `Covered-call floor: avoid selling below ${edge.executableCoveredCallFloor.toFixed(2)} unless call-away is acceptable.`,
    );
  if (edge?.executableCspCeiling != null)
    tradePlanNotes.push(
      `CSP ceiling: avoid selling above ${edge.executableCspCeiling.toFixed(2)} unless assignment is desired.`,
    );
  tradePlanNotes.push(
    "Use the base case while price remains inside the active OI range; green/red scenario paths are conditional and start at their activation rails.",
  );
  ...
  const magnetGapPct =
    magnet != null
      ? Math.abs((magnet - currentPrice) / currentPrice) * 100
      : null;
  const baseDriftDirection =
    baseTargetLast > currentPrice * 1.003
      ? "higher"
      : baseTargetLast < currentPrice * 0.997
        ? "lower"
        : "sideways";
  const baseCase =
    magnet != null && magnetGapPct != null && magnetGapPct > 8
  ...
  return {
    ...
    dominantExpiration,
    horizonSessions,
    forecastMode: useChainMagnetPath
      ? "classic_oi_path"
      : "smooth_pressure_path",
    maxDte: projection?.maxDte ?? null,
    includedExpirations:
      projection?.includedExpirations ??
      projectionPoints.map((point) => point.expiration),
    regime,
    confidence,
    pathBias,
    activeScenario,
